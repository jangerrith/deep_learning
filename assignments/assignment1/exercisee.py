...
# Specify regularisation parameter Lambda
Lambdas = [0, 0.01, 0.001]
...
# Set counter to enumerate cross-validation splits and regularization parameters
counter = 1
reg_counter = 1

# Evaluate NN for each regularisation parameter in Lambdas
for reg_param in Lambdas:

    print("Training for regularization parameter lambda = " + str(reg_param))
    ...
    for train_idx, val_idx in kfold.split(x_train):
        ...
        model = Sequential([
            Dense(256, activation='sigmoid', input_shape=(pixels,), kernel_initializer=keras.initializers.TruncatedNormal(
                seed=seed_), bias_initializer=keras.initializers.TruncatedNormal(seed=seed_), kernel_regularizer=regularizers.l2(reg_param)),
        ...
        plt.figure(reg_counter)
        plt.plot(history.history['loss'])
        plt.title(
            'model loss over epochs with regularization lambda = ' + str(reg_param))
        plt.ylabel('loss')
        plt.xlabel('epoch')
        plt.legend(['split 1', 'split 2', 'split 3',
                    'split 4', 'split 5'], loc='upper right')
        figname = "./graphs/loss_over_epochs_exercise_e_" + \
            str(reg_param) + ".png"
        plt.savefig(figname,
                    bbox_inches='tight')
